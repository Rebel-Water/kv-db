...
const std::string DB::filelockName = "filelock";
...
void DB::Merge()
{
    ...
    for (auto &dataFile : mergeFiles)
    {
        ...
        while (auto logRecord = ReadLogRecord(offset, dataFile))
        {
            ...
            if (pos.has_value() && pos->Fid == dataFile->FileId && pos->Offset == offset) // valid data
            {                                                                        // otherwise this k-v relation is not updated
                logRecord->Key = WriteBatch::logRecordKeyWithSeq(key, WriteBatch::NonTransactionSeqNo);
                auto mergePos = mergeDB.AppendLogRecord(logRecord.value());
    ...
    auto mergeFinishRecord = LogRecord(mergeFinishKey, Util::ToByteVector(std::to_string(nonMergeFileId)));

    auto encodeRecord = Code::EncodeLogRecord(mergeFinishRecord);
...
void DB::Put(const std::vector<byte> &key, const std::vector<byte> &value)
{
    ...
    LogRecord log_record(WriteBatch::logRecordKeyWithSeq(key, WriteBatch::NonTransactionSeqNo), value, LogRecordType::LogRecordNormal);
    auto pos = AppendLogRecord(log_record);
...
std::optional<LogRecord> DB::ReadLogRecord(int64 offset, const std::shared_ptr<DataFile>& datafile)
{
    ...
    LogRecord logRecord(std::move(key), std::move(value), header->recordType);
...
// merge except
LogRecordPos DB::AppendLogRecord(LogRecord& logRecord) // here appandlogrecord can be moved into datafile?
                                                                       // now i move appand and read all into db
{
    if (activeFile == nullptr) // when start db, activefile is null
        SetActiveDataFile();

    std::vector<byte> encodeRecord = Code::EncodeLogRecord(logRecord);
...
std::vector<byte> DB::GetValueByPosition(const LogRecordPos &pos)
{
    ...
    if (logRecord->Type == LogRecordType::LogRecordDeleted)
        throw std::runtime_error("DB::Get Error Key Not Found");

    return std::move(logRecord->Value);
}
...
void DB::LoadIndexFromDataFiles() // now kv record all in disk, read from it and load it into btree-memory
{
    ...
    std::unordered_map<uint64, std::vector<TransactionRecord>> transactionRecords;
    int currentSeqno = WriteBatch::NonTransactionSeqNo;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        while (true) // find fileId.data datafile to load in btree
        {                                                              // and read target.data in offset(int offset = 0)
            auto logRecord = ReadLogRecord(offset, datafile);
            if(logRecord == std::nullopt) break;
            int size = logRecord->Size;
            auto logRecordPos = LogRecordPos(fileId, offset);
            auto pair = WriteBatch::parseLogRecordKey(logRecord->Key);
            if (pair.second == WriteBatch::NonTransactionSeqNo)
            {
                updateIndex(pair.first, logRecord->Type, logRecordPos);
            }
            else
            {
                if (logRecord->Type == LogRecordType::LogRecordTxnFinsihed)
                {
                ...
                else
                {
                    ...
                    transactionRecords[pair.second].emplace_back(logRecord.value(), logRecordPos);
                }
            }
